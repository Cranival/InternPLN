...
// @ts-ignore
import { addIntern, addGalleryItem } from '../../lib/database.supabase';
// @ts-ignore
...
export function AddIntern() {
  ...
  const { mentors, isLoading: isDataLoading } = useData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (galleryFiles.length > 0) {
        galleryUrls = await Promise.all(galleryFiles.map(file => uploadToCloudinary(file)));
      }
      // Simpan data intern ke Supabase
      const newInternData = {
        ...formData,
        photo: photoUrl || 'https://images.unsplash.com/photo-1511367461989-f85a21fda167?w=400',
        status: 'pending',
      };
      const internResult = await addIntern(newInternData);
      const internId = internResult && internResult[0]?.id;
      // Simpan gallery ke Supabase
      if (internId && galleryUrls.length > 0) {
        for (const url of galleryUrls) {
          await addGalleryItem({
            intern_id: internId,
            photo: url,
            caption: '', // tambahkan caption jika ada
          });
        }
      }
      toast.success('Data peserta berhasil dikirim! Menunggu verifikasi dari mentor.', {
        duration: 4000,
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-8">
          <h1 className="mb-2 text-3xl font-bold text-blue-900 dark:text-white">
            Daftarkan Data Peserta
          </h1>
          <p className="text-gray-600 dark:text-gray-300">
            Isi formulir di bawah untuk mendokumentasikan peserta magang PLN
          </p>
        </div>
